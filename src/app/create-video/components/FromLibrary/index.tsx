...
export default function FromLibrary({
  ...
  const loadLibrary = async () => {
    ...
    try {
      // ✅ SECURE: Load library through API route with authentication
      ...
      const response = await fetch(
        `/api/list-library-images?userId=${userId}`,
        {
          method: 'GET',
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Could not load library');
      }
    ...
    <div className="method-content">
      ...
      <div className={`gallery-wrapper ${showSettings ? 'dimmed' : ''}`}>
        <div className="gallery-container">
          ...
          {displayCount < libraryImages.length && !showSettings && (
            <button
